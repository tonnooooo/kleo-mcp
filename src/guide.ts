...
import {
  ...
  SHOT_ACTION_MAX, SHOT_COVERS_MAX, SHOT_CAST_MAX, SHOT_TAG_MAX, VOICE_MAX,
...
${floorS > 0
    ? ` ONE PICTURE PER ${per} WORDS OF VOICE. In this film every picture becomes a paid clip of at least ${floorS} seconds: a scene shows one picture, a second one only when its line has ${2 * per} words or more (Kleo refuses one picture for a line that long, and cuts the pictures a line cannot pay for). The rhythm comes from what moves inside each shot — say it in "action" — not from cutting. Give every line ${per} words at the least: a shorter one still pays for a whole ${floorS}-second clip and shows only the part its voice fills. The hook and the closing too — a punch is a short first sentence inside the line, not a short line — and a narrator who speaks in short sentences says two or three of them to a line. A line still under ${per} words is joined by Kleo to its neighbour, word for word: the same narration, one clip fewer — but only while the film keeps more than two scenes, the joined line stays within ${VOICE_MAX} characters and the sections keep their colours apart. Otherwise the short line stays as written and its clip plays past its voice: write the words.`
...
4. WHAT KLEO REFUSES, BEFORE ANYTHING IS BILLED
 ...
 Total narration must fit the length: never more than about ${Math.round(words * 1.25)} words for ${dur}s, and not much under ${wordBudget(dur, 1.1).min} either — the film is as long as its voice, and a short narration makes a film shorter than asked.
