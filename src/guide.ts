...
export type GuideStyle = "cartoon" | "realistic" | "cyber" | "stickman";
...
export function buildGuide(o: GuideOptions): string {
  ...
  const keou = look ? (PICTURE_LOOKS.includes(look) ? "picture" : look === "stickman" ? "stickman" : "cinema") : "picture";
  const voiceLine = o.languages.map((l) => `${l}: ${(VOICES[l] ?? []).join("|")}`).join(" · ");

  const scenesSection = !look
    ? `${pictureSection("cartoon", dur)}

OTHER LOOKS: call kleo_storyboard_guide again with style "cyber" (motion design with icons and big type, no pictures — tech and security topics that want diagrams) or style "stickman" (a hand-drawn stickman acting the story, 9:16 only, on request) to get that look's vocabulary instead of this one.`
