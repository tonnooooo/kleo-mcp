...
const mcpApi: ExportedHandler<Env> = {
  async fetch(request, env, ctx) {
    ...
    if (!user) return json({ error: "account not found; reconnect Gatto" }, 401);
...
const app: ExportedHandler<Env> = {
  async fetch(request, env) {
    ...
    if (p === "/") {
      return json({
        name: "Gatto", description: "Remote MCP server that renders YouTube videos and Shorts.",
        mcp_endpoint: `${url.origin}/mcp`, transport: "streamable-http", auth: "oauth2.1",
        connect: { claude: "Settings → Connectors → Add custom connector", claude_code: `claude mcp add --transport http gatto ${url.origin}/mcp`, cursor: { mcpServers: { gatto: { url: `${url.origin}/mcp` } } } },
      });
    }
...
export default {
  ...
  scheduled: (_controller: ScheduledController, env: Env, ctx: ExecutionContext) => { ctx.waitUntil(tick(env)); },
