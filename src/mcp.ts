...
export function buildServer(env: Env, user: User, base: string): McpServer {
  ...
  server.registerTool("kleo_adapt_prompt", {
  ...
  }, async ({ prompt, duration_s, format, language, audience, tone, must_keep, style, music, subtitles, author }) => guarded(async () => {
    ...
    if (author !== "server") {
      ...
      const method = treatmentMethodText({ prompt: prompt.trim(), duration_s: dur, format: fmt, language: lang, look, sound }, v);
      void audit(env, user.id, null, "treatment.method", { variation: v.key, duration_s: dur, format: fmt, language: lang, look, music: brief.music?.wanted ?? null, subtitles: brief.subtitles });
      return ok({ ...base, treatment: null, author: "assistant", variation: v.key, ready_to_render: true, next: 'Write the treatment now, following the method in the text; then call kleo_create_video with prompt, duration_s, format, language, style (the look the treatment names), music and subtitles (the user\'s answers, as you passed them here) and the object as "treatment". If you cannot write it, call this tool again with author: "server".' },
    ...
    const r = await writeTreatment(env, { prompt: prompt.trim(), duration_s: dur, format: fmt, language: lang, look, sound });
    ...
    return ok({ ...base, style: r.treatment.look, treatment: r.treatment, ready_to_render: true, next: 'Show the user the logline and the decisions; then call kleo_create_video with prompt, duration_s, format, language, style (this treatment\'s "look"), music and subtitles (the user\'s answers) and this same object as "treatment".' },
  ...
  server.registerTool("kleo_get_result", {
    ...
    description: "Step 5. Download links for a finished video (film or animatic): the MP4, the thumbnail and the subtitles as an .srt file (burned-in subtitles and the music track, when the user asked for them, are inside the MP4).",
