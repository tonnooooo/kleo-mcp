...
export function summarizeFidelity(report: unknown, spec: RequestSpec | null, checksOf?: (pictureId: string) => string[] | null): FidelitySummary | null {
  ...
  const counted = (id: string) => !!id && id !== "style" && id !== "no-text";
...
export function buildServer(env: Env, user: User, base: string): McpServer {
  ...
  server.registerTool("kleo_adapt_prompt", {
    ...
    if (await isFlagActive(env, "plan_pause"))
      return ok({ ...base, treatment: null, ready_to_render: true, note: "planning quota exhausted" }, `${adaptivePromptText(brief)}${refsBlock(refs)}\n\nKleo cannot write the treatment right now: it has used up today's free planning. ${fallback}`);
    ...
    try {
      ...
      spec = sr.spec;
      void audit(env, user.id, null, "spec.adapt", { model: sr.model, attempts: sr.attempts, ms: sr.ms, ok: !!sr.spec, transient: sr.transient, mode: sr.spec?.mode ?? null, items: sr.spec?.items.length ?? 0, history: sr.history.slice(0, 3) });
    } catch (e) {
    ...
    void audit(env, user.id, null, "treatment.adapt", { model: r.model, attempts: r.attempts, ms: r.ms, usage: r.usage, est_neurons: r.est_neurons, ok: !!r.treatment, transient: r.transient, history: r.history.slice(0, 3), variation: r.treatment?.variation ?? null, spec: spec ? spec.mode : null });
