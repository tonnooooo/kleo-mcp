...
export function buildServer(env: Env, user: User, base: string): McpServer {
  ...
  server.registerTool("create_video", {
    ...
    description: "Start rendering a video from a template and a prompt. Returns immediately with a job_id and an ETA in minutes; the render runs on a GPU in the background. Credits are charged when the job is queued. Then tell the user the ETA and offer to check progress with get_job.",
    inputSchema: z.object({
    ...
    return ok({ ...view, message: `Queued. Estimated ${job.eta_min} minutes. ${job.credits} credit${job.credits > 1 ? "s" : ""} charged.` },
      `Job ${job.id} queued (template ${job.template}, ${view.format}, ${view.duration_s}s). Estimated ${job.eta_min} minutes. ${job.credits} credit(s) charged, ${fresh.credits - job.credits} left. Check progress later with get_job.`);
  }));
  ...
  server.registerTool("get_job", {
    ...
    inputSchema: z.object({ job_id: z.string().optional().describe("The job id returned by create_video. Omit to list recent jobs.") }),
    annotations: { readOnlyHint: true, idempotentHint: true, openWorldHint: false },
    ...
    const view = jobView(job);
    const human = job.state === "done" ? `Job ${job.id} is done. Call get_result for the download links.`
  ...
  server.registerTool("generate_thumbnail", {
    ...
    description: "Generate three alternative thumbnails from a finished job or from a text prompt. Returns a job_id; check it with get_job and fetch the files with get_result.",
    inputSchema: z.object({
    ...
    throw new JobError("Thumbnail generation is not enabled in this beta yet. The render already includes one thumbnail: see get_result.");
  }));
