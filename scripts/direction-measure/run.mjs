...
// The ledger (QUOTA.md, not .log: *.log is gitignored and the first version vanished). The free allocation is one bucket for every session on the account and none of them can see the
...
// of them died on its first request. A line here costs nothing and says who spent what.
{
  ...
  const ledger = new URL("./QUOTA.md", import.meta.url);
