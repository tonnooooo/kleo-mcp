...
function run(name, opts) {
  let m = [1, 0, 0, 1, 0, 0], stack = [], lo = 0, hi = 0, w = 6;
  const mul = (a, b) => [a[0]*b[0]+a[2]*b[1], a[1]*b[0]+a[3]*b[1], a[0]*b[2]+a[2]*b[3], a[1]*b[2]+a[3]*b[3], a[0]*b[4]+a[2]*b[5]+a[4], a[1]*b[4]+a[3]*b[5]+a[5]];
  const pt = (x, y) => { if (!Number.isFinite(x) || !Number.isFinite(y)) return; const yy = m[1]*x + m[3]*y + m[5]; lo = Math.min(lo, yy - w); hi = Math.max(hi, yy + w) };
  const ctx = new Proxy({}, { get(_, k) {
    switch (k) {
      ...
      case "arc": return (x, y, r) => { pt(x, y - r); pt(x, y + r) };
  ...
  return { up: Math.round(-lo), down: Math.round(hi) };
}
...
for (const n of names) {
  ...
  table[n] = { up: Math.max(a.up, b.up), down: Math.max(a.down, b.down) };
}
console.log(JSON.stringify(table));
console.error(Object.entries(table).map(([k, v]) => `${k.padEnd(12)} up ${String(v.up).padStart(4)}  down ${String(v.down).padStart(4)}`).join("\n"));
