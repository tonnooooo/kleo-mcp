...
for(const f of [file,resolve(build,'timeline.json'),resolve(engine,'film.js'),resolve(engine,'film.html'),resolve(engine,'render.mjs'),resolve(engine,'stickman.js'),resolve(engine,'cinema.js'),resolve(engine,'picture.js'),resolve(engine,'sketch.js'),resolve(engine,'assets/font.ttf'),resolve(engine,'assets/mono.ttf'),resolve(engine,'assets/cartoon.ttf'),resolve(engine,'assets/real.ttf'),resolve(engine,'assets/world.json'),...readdirSync(resolve(engine,'modes')).filter(f=>f.endsWith('.js')).sort().map(f=>resolve(engine,'modes',f)),...project.scenes.filter(s=>s.image).map(s=>resolve(dir,s.image)),...project.scenes.flatMap(s=>(s.shots||[]).filter(x=>x.image).map(x=>resolve(dir,x.image)))].filter(existsSync))hash.update(readFileSync(f));
