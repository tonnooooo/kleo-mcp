...
// A part or the master is valid on its FRAME COUNT, its size and its rate within a hundredth: r_frame_rate is ffprobe's
// GUESS of the rate from the timestamps, and a master concatenated from parts drawn over a 59.94 footage track
// (kie.ai's MiniMax) came back as "60000/1001" with exactly the right number of frames at exactly the right size —
// five finish rentals of gt_nyhb8aj9 (22 September 2026) lost to a string comparison. The frame count is the check
// that means something: 1089 frames muxed with a 60 fps mix ARE the 60 fps film.
const rateOf=v=>{const [n,d]=String(v||'0/1').split('/').map(Number);return d?n/d:0};
const validPart=(p,count)=>{try{const v=probe(p).streams.find(s=>s.codec_type==='video');return Number(v.nb_read_frames)===count&&v.width===width&&v.height===height&&Math.abs(rateOf(v.r_frame_rate)-fps)<0.1}catch{return false}};
