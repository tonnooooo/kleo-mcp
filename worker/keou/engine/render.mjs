...
for(const f of [file,resolve(build,'timeline.json'),resolve(engine,'film.js'),resolve(engine,'film.html'),resolve(engine,'render.mjs'),resolve(engine,'stickman.js'),resolve(engine,'cinema.js'),resolve(engine,'assets/font.ttf'),resolve(engine,'assets/mono.ttf'),resolve(engine,'assets/world.json'),...readdirSync(resolve(engine,'modes')).filter(f=>f.endsWith('.js')).sort().map(f=>resolve(engine,'modes',f)),...project.scenes.filter(s=>s.image).map(s=>resolve(dir,s.image))])hash.update(readFileSync(f));
