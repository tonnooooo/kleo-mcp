...
def main(project):
    ...
    decode=subprocess.run([ffmpeg,'-nostdin','-v','info','-i',str(movie),'-vf','scale=270:-2,blackdetect=d=0.15:pix_th=0.02','-an','-f','null','-'],capture_output=True,text=True,check=True)
    ...
    if not -18.5<=lufs<=-14 or peak>-.8:raise ValueError(f'Audio levels outside target: {lufs} LUFS, {peak} dBTP')
