...
| Crediti gratis | `FREE_CREDITS` = 2 per ogni nuovo account | 2 Short interi. Un video lungo ne costa 3, quindi con la prova gratuita si possono fare solo Short: il percorso più economico e più collaudato |
...
| Crediti | 1 per uno Short (fino a 90 s) · 3 per un video fino a 5 minuti · +1 per ogni minuto in più | scalati alla messa in coda; restituiti **per intero** se il video fallisce, se resta in coda più di `QUEUE_MAX_WAIT_MIN` (3 ore) o se viene annullato prima di partire. Se lo si annulla a render iniziato torna indietro solo la parte non ancora renderizzata (`refundFor` in `src/jobs.ts`): uno Short annullato oltre il 50% non restituisce niente |
