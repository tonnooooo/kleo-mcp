...
| Modello del planner (20 set) | `PLAN_MODEL` = `claude-opus-5` in wrangler.jsonc, acceso SOLO dal segreto `ANTHROPIC_API_KEY` (`npx wrangler secret put ANTHROPIC_API_KEY`); senza segreto resta `AI_MODEL` (llama-4-scout 17B) | Il 17B scrive film piatti (43 parole, "La dolce pasticcera lavora nella sua cucina", nomi da scaletta). Con la chiave ogni chiamata di pianificazione (treatment, direction, scaletta, scene, passo inglese) va all'API Anthropic (`callClaude` in src/storyboard.ts, SDK ufficiale): circa 0,30 $ a film su opus-5, 0,10 $ su `claude-sonnet-5`, contro ~0,01 $ del 17B. Errori 429/5xx sono "transient" come quelli di Workers AI (pausa 15 min), un rifiuto è un errore normale del tentativo |
