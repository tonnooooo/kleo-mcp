...
| Prova gratuita | `FREE_FILMS` = **0** (dal 14 set sera) | Nessun film gratis: un conto nuovo parte da 0 crediti e il primo film si paga (il pacchetto da 5 EUR = 10 crediti compra un film da 20 s). Un valore > 0 regala tanti film al prezzo minimo (`freeCreditsFor` in `src/templates.ts`). Era 1 dal 13 set: ogni sconosciuto costava 2-4 $ di clip kie.ai, e senza abbonamento non si regge |
