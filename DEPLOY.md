...
| Fedeltà delle immagini (20 set) | `notEnglish`/`foreignPictureFields` in `src/direction.ts`, passo 0b e 2b del planner in `src/storyboard.ts`, `lights_pictures`/`cast_for` in `worker/kleo_pictures.py` | Il 19 set l'animatic di una tester (gt_ad2musq5, "pasticcera magra, capelli biondi corti e raccolti, grembiule lilla") è uscito con tre donne diverse, ricce e col grembiule rosso: i prompt delle immagini e l'aspetto del cast arrivavano al modello **in italiano** (CLIP legge solo inglese) e la "luce d'accento" rossa della sezione tingeva tutto il fotogramma. Ora: ogni `image_prompt`, il mondo, nomi e aspetto del cast, oggetti e vietati della direction sono **sempre in inglese** (il planner li chiede così e poi li traduce in una chiamata sola se il modello non obbedisce; uno storyboard scritto dall'assistente in italiano viene rifiutato a parole); l'aspetto del cast va **in testa** al prompt della GPU, la luce d'accento **non entra** nelle immagini del look animation. Stesso giorno: le regole di ritmo del validatore (classe di movimento, scala, direzione, mosse forti) sono **avvisi**, non rifiuti — due job erano morti in pianificazione su "STILL repeats" che nessuno poteva risolvere (mani e bambini forzano lo static hold su ogni inquadratura) — e la pianificazione ha un tetto: 90 s a chiamata, 4 minuti a tentativo (`MODEL_CALL_TIMEOUT_MS`, `PLAN_BUDGET_MS`), dopo un tentativo sparito senza traccia per 13 minuti. Un nuovo video con lo stesso prompt entro 3 ore riusa il treatment del precedente se il client non lo rimanda (audit `treatment: reused`) |
| Modello del planner (20 set) | `PLAN_MODEL` = `anthropic/claude-sonnet-5` + `PLAN_API_URL` = OpenRouter in wrangler.jsonc, accesi SOLO dal segreto `PLAN_API_KEY` (`npx wrangler secret put PLAN_API_KEY`); in alternativa `ANTHROPIC_API_KEY` per un modello `claude-…` diretto; senza segreti resta `AI_MODEL` (gpt-oss-120b) | Connettore compatibile OpenAI (`callOpenAICompat` in src/storyboard.ts, fetch puro): una chiave OpenRouter apre Sonnet/Opus, GPT-5, Gemini, DeepSeek; si confrontano con `node scripts/film-make.mjs "<richiesta>" --model <id>` dopo `set -a; . ./.secrets.local; set +a`. Sonnet 5 ~0,10 $ a film, gpt-5-mini ~0,03, deepseek ~0,01. Errori 429/5xx sono "transient" (pausa 15 min), un errore nel corpo è un errore del tentativo |
| Strada kie.ai per Claude (20 set, PRONTA, SPENTA) | `ANTHROPIC_BASE_URL` = `https://api.kie.ai/claude` + `ANTHROPIC_AUTH` = `bearer` in wrangler.jsonc, segreto `ANTHROPIC_API_KEY` = chiave kie.ai, `PLAN_MODEL` = `claude-sonnet-5`, e `PLAN_API_URL` TOLTO (la strada OpenRouter vince se ha la sua chiave) | Stesso Messages API attraverso il proxy di kie.ai (`callClaude` con baseURL + Bearer, senza `output_config`). Il 20 set kie.ai rispondeva `530 Internal error` su claude-sonnet-5 e "The API key is not authorized to use this model" su ogni altro chat model (GPT 5.x, Haiku, Sonnet 4.5, Opus): la chiave del proprietario non è abilitata ai chat model. Prova prima: `curl -X POST https://api.kie.ai/claude/v1/messages -H "Authorization: Bearer $KIE_API_KEY" -H "anthropic-version: 2023-06-01" -H "content-type: application/json" -d '{"model":"claude-sonnet-5","max_tokens":32,"messages":[{"role":"user","content":"Reply OK"}]}'` deve rispondere `"type":"message"`; poi `npx wrangler secret put ANTHROPIC_API_KEY`, le tre variabili, `npx wrangler secret delete PLAN_API_KEY`, deploy, `scripts/animatic-e2e.mjs` |
| Planner = gpt-oss-120b (20 set, sera) | `AI_MODEL` = `@cf/openai/gpt-oss-120b`, `PLAN_BUDGET_MIN` = 8 in wrangler.jsonc; `callModel` in src/storyboard.ts lo chiama senza json_schema, con `reasoning.effort: low` e sei volte i token; il system prompt gli dice di NON contare i caratteri | Misurato sullo stesso film (Star Wars 30 s): llama-4-scout scrive "A young warrior receives a transmission on a ruined desert planet"; gpt-oss scrive "A warning flickers across the crystal: the enemy lives now" e un cast che si può disegnare. Costo ~1700 neuroni (2 centesimi) a film, 3-4 minuti di pianificazione contro 1. Con json_schema o con "count characters" nel prompt bruciava tutti i token contando lettere e tornava vuoto (2 chiamate su 3): senza, 4 su 4 in 21-36 s. Il gate di fedeltà (must_keep non detto) è ora un avviso: il planner lo rimanda indietro una volta e riporta ciò che resta; uno storyboard scritto dal client viene ancora rifiutato |
| Immagini realistic = RealVisXL V5.0 (20 set) | `worker/kleo_pictures.py` MODELS, `worker/prewarm_models.py` | Confronto su GPU noleggiata (stessi prompt e semi): SDXL base = volti cerosi e sfocati (lo Short "Star Wars" gt_6xchnk99), RealVisXL V5.0 = pelle vera e stesso volto tra i ritratti, Juggernaut XL v9 = bello ma cambia capelli e ignora il costume. Scaricato sulla macchina al primo job realistic (~7 GB, 1-2 min), come prima il base. Dettagli in docs/NITIDEZZA.md |
...
| Pre-volo kie.ai (15 set) | `kiePreflight` in `src/footage.ts`, chiamato da `createJob` per ogni film sulla strada kie | Il 15 set quattro film hanno noleggiato una scheda, disegnato i fotogrammi e doppiato lo script per poi scoprire che kie.ai aveva 0,07 $. Ora il saldo (`GET /api/v1/chat/credit`) si legge **alla creazione** contro il costo stimato del film (inquadrature dello storyboard × secondi × prezzo del modello; senza storyboard ~1 inquadratura ogni 3 s, min 6, max il tetto): se non basta il film viene rifiutato subito a parole, niente GPU, audit `footage.preflight` con `owner_action: top up kie.ai`, e l'animatic viene offerto. Il silenzio di kie.ai non rifiuta mai. Il negozio resta aperto: il proprietario ricarica kie.ai quando qualcuno paga |
...
| Due prodotti (15 set) | **film** (ogni inquadratura una clip kie.ai, prezzo a lunghezza) · **animatic** (`ANIMATIC_CREDITS` = **5** fissi, fino a `ANIMATIC_MAX_S` = 60 s) | Regola del proprietario del 15 set: le clip le paga lui su kie.ai, quindi **il film si fa solo a chi ha pagato** (una riga in `payments` con Stripe, `hasPaid` in `src/db.ts`): crediti regalati, bonus o scritti a mano — compreso il suo conto di prova — comprano l'animatic e mai un film. L'**animatic** è lo stesso treatment, direction, storyboard e gli stessi fotogrammi disegnati sulla GPU, con la camera che si muove su ciascuno (`picture.js` kenBurns), lo stesso layer e narratore, 4K 60 fps, **nessuna clip generata**: costa ~0,05 $ di GPU e zero kie.ai. È un parametro del job (`params.product`), non un template: `kleo_create_video` prende `product: "film" \| "animatic"`, `kleo_account` dice `has_paid`. Un film chiesto da un conto non pagante viene rifiutato a parole (con l'animatic come via d'uscita), niente addebito. Per far entrare un tester come cliente: una riga in `payments` (`session_id` sintetico `manual_…`, `payment_intent` NULL, `status 'paid'`) |
| Prova gratuita | `FREE_CREDITS` = **7** (dal 14 set sera) | La prova gratuita resta (0 EUR, un bottone, niente da scrivere): con i 7 regalati si fa **un animatic** (5), mai un film (il più corto costa 10 e comunque richiede un pacchetto acquistato); col pacchetto da 5 EUR (10 crediti) si arriva a 17, uno Short da 30 s, e il film si apre. Il vecchio `FREE_FILMS` (regalo = un film al prezzo corrente) è sparito perché faceva l'opposto |
...
| Crediti | **1 credito = 2 secondi di film, minimo 10** (`filmBase`/`tariffSentence()` in `src/templates.ts`, dal 14 set): 20 s = 10 · 30 s = 15 · 60 s = 30 · 90 s = 45 · 5 min = 150; **animatic 5 fissi** (`creditsForProduct`). Copre il costo vivo (~0,13 $/s di clip kie.ai + GPU) con margine 1,5-1,9× ai prezzi dei pacchetti (0,40-0,50 EUR a credito). Prima: 7 fino a 90 s (3,50 EUR per un film che costava 11 $) | scalati alla messa in coda; restituiti **per intero** se il video fallisce, se resta in coda più di `QUEUE_MAX_WAIT_MIN` (3 ore) o se viene annullato prima di partire. Se lo si annulla a render iniziato torna indietro solo la parte non ancora renderizzata (`refundFor` in `src/jobs.ts`) |
