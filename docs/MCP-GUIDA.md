...
`kleo_create_video` accetta anche `style`: **realistic** (film girato, fotografia cinematografica) e, dal 14 settembre 2026, **animation** (film animato 2D: sfondi dipinti, personaggi disegnati una volta e ripetuti uguali), entrambi sulla stessa pipeline (fotogramma di riferimento → clip kie.ai → 4K 60 fps) e allo stesso prezzo; omesso, decide il treatment (passo 0 del metodo, vedi `docs/ANIMAZIONE.md`). I vecchi look cartoon, cyber e stickman restano nel codice ma non si vendono più. Cartoon e realistic usano lo stile Keou `picture` (vedi `docs/PICTURE-STYLE.md`): ogni scena si divide in `shots`, da due a quattro immagini a tutto schermo, ognuna con il suo `image_prompt` e con `at`, la parola della narrazione su cui l'immagine cambia; la scena di chiusura ne ha una sola. Oggi le disegna tutte la GPU noleggiata su Vast (`IMAGE_SERVER_MAX=0`), così la quota gratuita giornaliera di Workers AI resta intera per gli storyboard; ogni immagine ha un movimento lento, una sfumatura scura in basso per i sottotitoli e, quando serve, una scritta grande di due o tre parole. Niente icone, niente HUD, niente beats: un video sui pirati mostra spiagge, sabbia e velieri, uno sullo spazio razzi e stazioni.
